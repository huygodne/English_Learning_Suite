...
const LessonList: React.FC<LessonListProps> = ({ lessons = [], progressByLesson = {}, loadingProgress = false }) => {
  ...
  return (
    <motion.div
      key={`lesson-list-${lessons.length}-${lessons[0]?.id || 0}`}
      ...
      {lessons.map((lesson, index) => {
        ...
        return (
          <motion.div
            ...
            className="bg-white rounded-3xl shadow-lg border border-slate-100 p-6 group hover:shadow-xl transition-all duration-300 flex flex-col h-full"
          >
            {/* Header with badges */}
            <div className="flex items-center justify-between mb-4 flex-shrink-0">
              <span className="bg-blue-100 text-blue-700 text-sm font-semibold px-3 py-1.5 rounded-full whitespace-nowrap">
                Bài {lessonNumber}
              </span>
              <span className={`text-sm font-semibold px-3 py-1.5 rounded-full whitespace-nowrap ${level.classes}`}>
                {level.label}
              </span>
            </div>

            {/* Title */}
            <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-4 group-hover:text-primary-600 transition-colors duration-300 line-clamp-3 break-words min-h-[4.5rem]">
            ...
            {/* Time info */}
            <div className="flex items-start gap-2 text-sm text-slate-600 mb-6 flex-shrink-0">
            ...
            <Link
              ...
              className="block w-full text-center px-6 py-3 rounded-2xl bg-blue-600 text-white font-semibold hover:bg-blue-700 transition-all duration-300 hover:scale-105 mt-auto flex-shrink-0"
            >
              {status.action}
            </Link>
          </motion.div>
