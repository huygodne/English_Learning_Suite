  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'password') {
      ...
      if (value.length > 0) {
        ...
        const hasOtherValue = (name === 'username' && (formData.fullName || formData.phoneNumber)) ||
                              (name === 'fullName' && (formData.username || formData.phoneNumber)) ||
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl shadow-2xl p-6 transform transition-all duration-300 hover:scale-[1.02] hover:bg-white/15">
            ...
            <div className="text-center">
              <p className="text-sm text-white/70">
                Đã có tài khoản?{' '}
                <Link to="/login" className="font-medium text-blue-300 hover:text-blue-200 transition-colors duration-300">
                  Đăng nhập ngay
                </Link>
              </p>
            </div>
